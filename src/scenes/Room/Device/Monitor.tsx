...
const MONITOR_CONTENT_URL = "/project2/index.html";
...
const Monitor: React.FC<MonitorProps> = ({ roomGLTF }): JSX.Element => {
  ...
  const [monitorScreenGeometry, setMonitorScreenGeometry] =
    useState<Box3 | null>(null); // monitor screen geometry from monitor model, will be set after the model is loaded
  ...
  // load monitor model
  const { nodes: monitorNodes, materials: monitorMaterials } = useGLTF(
    SCENE_MANAGER.SCENE_ASSETS.models.room.monitor,
  ) as unknown as MonitorGLTF;
  ...
  // exit manual fullscreen when zooming out of the room
  useEffect(() => {
    if (sceneZoomed === "out") {
  ...
  const html = (
      ...
      {...{
        ...(isFullscreen
          ? {
              ...
              position: [
                deskPosition.z +
                  DEVICE.MONITOR.POSITION_OFFSET.z -
                  0.161 +
                  0.05,
                deskPosition.y + DEVICE.MONITOR.POSITION_OFFSET.y + 0.717,
                0,
          ...
          : {
              ...
              transform: true,
              position: [
                HTML_POSITION_X_OFFSET,
                monitorScreenGeometry
                  ? monitorScreenGeometry.max.y + HTML_POSITION_Y_OFFSET
                  : HTML_DEFAULT_Y_POS,
                0,
              ],
    ...
    >
      <div
        className={`${styles["monitor-container"]} ${isFullscreen ? styles["fullscreen"] : ""}`}
      >
        {/* scroll hint */}
        {/* {showScrollHint && (
        ...
        <div className={styles["gnome-panel"]}>
          <div className={styles["panel-left"]}>
            <div className={styles["activities-button"]}>Activities</div>
          </div>
  ...
  return (
    <group
      ...
      dispose={null}
      position={[
        deskPosition.x + DEVICE.MONITOR.POSITION_OFFSET.x,
        DEVICE.MONITOR.POSITION_OFFSET.y,
        deskPosition.z + DEVICE.MONITOR.POSITION_OFFSET.z,
      ]} // set relative to desk position
      ...
      <group rotation={[-Math.PI, 0, 0]} scale={[1, 2.006, 2.006]}>
        <group rotation={[Math.PI / 2, 0, 0]}>
          ...
          >
            {html}
            <mesh
              geometry={monitorNodes.Object_4.geometry}
              material={monitorMaterials.plastic}
            />
            <mesh
              geometry={monitorNodes.MonitorScreen.geometry}
              material={monitorMaterials["Material.040"]}
            ></mesh>
          </group>
          <mesh
            geometry={monitorNodes.Object_7.geometry}
            material={monitorMaterials["Material.001"]}
